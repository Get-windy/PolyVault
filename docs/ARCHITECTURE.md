...
**版本**: v4.1（插件化架构 + 权限管理）  
**最后更新**: 2026-03-21 12:10  
**状态**: 开发中  
**核心理念**: 核心最小化 + 插件化一切  
**基于技术**: eCAL + 四大插件接口 + 系统插件 + 生态插件 + 权限管理器
...
- [插件架构文档](./PLUGIN_ARCHITECTURE.md) - 插件系统详细设计
- [插件开发指南](./PLUGIN_DEVELOPMENT_GUIDE.md) - 如何开发插件
- [eCAL设计文档](./ECAL_DESIGN.md) - 端对端通信组件设计
- [技术规格说明](./TECHNICAL_SPECIFICATION.md) - 完整技术方案
...
│  │  ├─ 端对端通信组件（eCAL）                        │ │
│  │  ├─ 四大插件接口                                  │ │
│  │  ├─ 插件管理器（PluginManager）【新增 v4.1】     │ │
│  │  ├─ 权限管理器（PermissionManager）【新增 v4.1】 │ │
│  │  └─ 数据总线（DataBus）【新增 v4.1】             │ │
│  └───────────────────────────────────────────────────┘ │
│                                                         │
│  ┌───────────────────────────────────────────────────┐ │
│  │  系统插件（高权限，可移除）                        │ │
...
public:
    ...
    PluginState getPluginState(const std::string& plugin_id);
    
    // 消息路由
    void routeMessage(const std::string& target, const Message& msg);
};
```
...
### 5. 数据总线（DataBus）【新增 v4.1】

**职责**: 提供基于eCAL的安全发布-订阅通信能力
...
enum class MessageType : int {
    KEY_EXCHANGE = 1,        // 密钥交换
    SIGNATURE_REQUEST = 2,   // 签名请求
    SIGNATURE_RESPONSE = 3,  // 签名响应
    VERIFICATION_REQUEST = 4,// 验证请求
    VERIFICATION_RESPONSE = 5,// 验证响应
    HEARTBEAT = 6,           // 心跳
    CONFIG_SYNC = 7,         // 配置同步
    KEY_ROTATION = 8         // 密钥轮换
};
...
struct SecureMessage {
    ...
    std::string receiver_id;             // 接收者ID
    std::vector<uint8_t> encrypted_payload; // 加密负载
...
struct DataBusConfig {
    ...
    std::string network_interface;// 网络接口
    uint16_t port;                // 端口号（0表示自动选择）
    bool enable_encryption;       // 启用加密
    uint32_t max_message_size;    // 最大消息大小（默认1MB）
    uint32_t timeout_ms;          // 超时时间（默认5秒）
};
```
...
public:
    ...
    bool isNodeConnected(const std::string& node_id);
...
│                                                                 │
│  返回路径（逆向）                                                 │
│  硬件安全模块 → zk_vault → DataBus → PermissionManager → Flutter UI │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
